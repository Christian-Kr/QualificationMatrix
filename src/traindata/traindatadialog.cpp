//
// traindatadialog.cpp is part of QualificationMatrix
//
...
#include "traindatadialog.h"
#include "ui_traindatadialog.h"
...
TrainDataDialog::TrainDataDialog(QWidget *parent)
    : QDialog(parent), ui(new Ui::TrainDataDialog)
{
    ui->setupUi(this);
    ...
    connect(applyButton, &QPushButton::clicked, this, &TrainDataDialog::apply);
}

TrainDataDialog::~TrainDataDialog()
{
    delete ui;
}

void TrainDataDialog::closeEvent(QCloseEvent *event)
{
    saveSettings();
...
void TrainDataDialog::saveSettings()
{
...
void TrainDataDialog::apply()
{
    // Test whether data train model is dirty or not.
    auto dm = QMDataManager::getInstance();
    if (dm->getTrainDataModel()->isDirty()) {
        QMessageBox::StandardButton res = QMessageBox::question(
            this, tr("Schulungsdaten - Speichern"), tr(
                "Die Änderungen werden in die Datenbank geschrieben."
                "\n\nSind Sie sicher?"
...
void TrainDataDialog::reject()
{
    ...
    if (dm->getTrainDataModel()->isDirty()) {
        QMessageBox::StandardButton res = QMessageBox::question(
            this, tr("Schulungsdaten - Zurücksetzen"), tr(
