//
// qualiresultmodel.cpp is part of QualificationMatrix
//
...
QualiResultModel::QualiResultModel(QObject *parent)
    ...
    trainExceptionModel(nullptr), resultRecords(new QList<QualiResultRecord *>())
{
}

QualiResultModel::~QualiResultModel() {}

void QualiResultModel::updateModels()
{
    QMDataManager *dm = QMDataManager::getInstance();
...
void QualiResultModel::setEmployeeFilter(const QString &filter)
{
    employeeFilter = filter;
}

void QualiResultModel::setTrainFilter(const QString &filter)
{
    trainFilter = filter;
}

void QualiResultModel::setFuncFilter(const QString &filter)
{
    funcFilter = filter;
}

void QualiResultModel::resetModel()
{
...
bool QualiResultModel::updateQualiInfo(
    ...
    for (int i = 0; i < employeeModel->rowCount(); i++)
    {
        ...
        for (int j = 0; j < employeeFuncModel->rowCount(); j++)
        {
            ...
            for (int k = 0; k < qualiModel->rowCount(); k++)
            {
                ...
                QualiResultRecord *record = new QualiResultRecord(this);
...
QVariant QualiResultModel::headerData(int section, Qt::Orientation orientation, int role) const
{
...
bool QualiResultModel::setHeaderData(
...
int QualiResultModel::rowCount(const QModelIndex &) const
{
    return this->resultRecords->size();
}

int QualiResultModel::columnCount(const QModelIndex &) const
{
    return 8;
}

QVariant QualiResultModel::data(const QModelIndex &index, int role) const
{
    if (!index.isValid())
    {
        return QVariant();
...
int QualiResultModel::qualiStateRowFromFuncTrain(const int &funcRow, const int &trainRow) const
{
...
int QualiResultModel::getIntervallFromTrain(const QString &train)
{
...
bool QualiResultModel::setData(const QModelIndex &index, const QVariant &value, int role)
{
    return QAbstractTableModel::setData(index, value, role);
}

Qt::ItemFlags QualiResultModel::flags(const QModelIndex &index) const
{
    if (!index.isValid())
    {
